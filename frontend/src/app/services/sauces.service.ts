...
export class SaucesService {
  ...
  likeSauce(id: string, like: boolean) {
    return new Promise((resolve, reject) => {
      this.http.post(`${apiURL}/api/sauces/${id}/like`,
        {
  ...
  createSauce(sauce: Sauce, image: File) {
    return new Promise((resolve, reject) => {
      ...
      this.http.post("https://appsopekocko.herokuapp.com/api/sauces", formData).subscribe(
        (response: { message: string }) => {
          resolve(response);
        },
        (error) => {
          console.log("SaucesService ~ error", error)
          reject(error);
        }
      );
    });
  ...
  modifySauce(id: string, sauce: Sauce, image: string | File) {
    return new Promise((resolve, reject) => {
      if (typeof image === 'string') {
        this.http.put("https://appsopekocko.herokuapp.com/api/sauces/" + id, sauce).subscribe(
          (response: { message: string }) => {
            resolve(response);
          },
          (error) => {
